      ...
      <div className="grid gap-3 py-7 sm:grid-cols-3">
        {[
          ...
          >
            ...
            <p className="display mt-2 text-4xl font-black">{value}</p>
          </div>
        ))}
      </div>

      <div className="mb-6 flex border-b border-[#292929]">
        <Link
          href="/my-plan"
        ...
        >
          Today&apos;s Plan <span className="ml-1 text-[#888]">{plan.length}</span>
        </Link>
        <Link
        ...
        >
          Saved <span className="ml-1 text-[#888]">{saved.length}</span>
        </Link>
      </div>
      ...
      <div className="flex min-w-0 flex-1 flex-col p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 text-sm text-[#777]">{w.equipment}</p>
          </div>
        ...
        <div className="mt-auto flex flex-wrap items-center justify-between gap-4 pt-6">
          <div className="flex gap-4 text-xs text-[#888]">
            <span className="flex items-center gap-1">
              <Clock3 size={14} /> {w.duration}m
            </span>
            <span className="flex items-center gap-1">
              <Flame size={14} /> {w.caloriesBurned} kcal
            </span>
            <span className="flex items-center gap-1">
              <Star size={14} className="text-[#ccff00]" /> {w.rating}
            </span>
          </div>
    ...
    <Suspense
      fallback={
        <section className="container min-h-[70vh] py-20">
          <p className="text-sm font-bold text-[#777]">Loading workouts…</p>
        </section>
      }
    >
      <PlanInner />
